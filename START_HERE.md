...
This file is the one an agent should read first when helping a user build a custom Hectic extension.
...
- you already have a local or staging Hectic instance
...
```bash
hectic auth login --hectic-url https://hectic.yourdomain.com
hectic workspaces list
hectic extensions install . --workspace ws_sandbox
hectic extensions list --workspace ws_sandbox
hectic extensions validate EXTENSION_ID
hectic extensions activate EXTENSION_ID
hectic extensions monitor --id EXTENSION_ID
hectic extensions skills list --id EXTENSION_ID
```

The helper scripts wrap the same lifecycle for agents:

```bash
export HECTIC_URL=https://hectic.yourdomain.com
export HECTIC_WORKSPACE_ID=ws_sandbox
./scripts/install-into-sandbox.sh .

export HECTIC_EXTENSION_ID=ext_installed_id
./scripts/verify-extension.sh
./scripts/upgrade-in-sandbox.sh .
```
...
```bash
hectic extensions upgrade . --id EXTENSION_ID
hectic extensions monitor --id EXTENSION_ID
```
...
```bash
hectic extensions deactivate EXTENSION_ID
```

## Review Rule

Do not activate a self-built extension outside a sandbox or staging workspace until both of these are complete:
